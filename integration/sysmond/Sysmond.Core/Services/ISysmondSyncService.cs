...
public interface ISysmondSyncService
{
    ...
    /// <summary>
    /// Stock-receipt (Entry/Exit/Transfer) → <c>stock_transactions</c> upsert; dönem-scoped orphan.
    /// Inventory miktarı değişmez (kaynak: stock/balance sync). Outbound create yok.
    /// </summary>
    Task<SysmondStockTransactionSyncResult> SyncStockTransactionsAsync(
        Guid sysmondCompanyId,
        string accessToken,
        CancellationToken cancellationToken = default);
    ...
    /// <summary>Sysmondax <c>POST /api/app/act/act</c> + yerel Act oluşturma.</summary>
    Task<SysmondActResponse> CreateActAsync(
        Guid sysmondCompanyId,
        string accessToken,
        SysmondCreateActRequest request,
        CancellationToken cancellationToken = default);

    /// <summary>Sysmondax <c>POST /api/app/warehouse</c> + yerel Warehouse oluşturma.</summary>
