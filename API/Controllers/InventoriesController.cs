...
public class InventoriesController : ControllerBase
{
    ...
    public async Task<ActionResult<IReadOnlyList<InventoryResponse>>> GetAll(CancellationToken cancellationToken)
        => Ok(await _inventoryService.GetAllAsync(cancellationToken));
    ...
    public async Task<ActionResult<InventoryResponse>> GetByProductAndWarehouse(Guid productId, Guid warehouseId, CancellationToken cancellationToken)
    {
    ...
    public async Task<ActionResult<IReadOnlyList<InventoryResponse>>> GetByWarehouse(Guid warehouseId, CancellationToken cancellationToken)
        => Ok(await _inventoryService.GetByWarehouseIdAsync(warehouseId, cancellationToken));

    /// <summary>Ürünün tüm depolardaki stoklarını listeler.</summary>
    [HttpGet("by-product/{productId:guid}")]
    public async Task<ActionResult<IReadOnlyList<InventoryResponse>>> GetByProduct(Guid productId, CancellationToken cancellationToken)
        => Ok(await _inventoryService.GetByProductIdAsync(productId, cancellationToken));
}
